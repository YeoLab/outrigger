...
If you get an error when you install that ends with:
...
This is because Unicode is stupid and you have to set the language to use
Unicode (i.e. characters with accents or greek letters, vs ASCII that has none
of that) explicitly. For example, if you're on a US Linux machine, you can fix
this by exporting the locale and language to ``en_US.utf-8``. For other
langauges, for example on a Japanese machine, you would replace ``en_US.utf-8``
with ``ja_JP.UTF-8``.
